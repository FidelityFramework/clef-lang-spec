...
Clef quotations (`<@ ... @>`) are **compile-time inspectable data structures**, an intrinsic form of the language ([Expressions, Quoted Expressions](expressions.md)). Unlike regular code, which compiles to instructions, a quotation is elaborated into the program graph as data the compiler reads during compilation; it has no run-time value and is never evaluated.

This makes quotations the form for carrying binding metadata:
- Generated by Farscape from C/C++ headers
- Compiled with the program, as Clef source
- Read at compile time by CCS, structurally, by type name and field name
- Never executed at runtime, and never referenced from executed code (CCS8066)
