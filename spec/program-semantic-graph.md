...
| `Observable`, `Incremental` | Reactive observation and demand-driven computation | Compiler |

The [`Signal`/`Memo`/`Effect` surface](reactive-signals.md), including batching, desugars to these intrinsic semantics. It does not dispatch to a separate reactive library engine. The PSG preserves a compiler-visible plan; active graph instances, cached state and owned subscriptions remain runtime responsibilities of the selected target.
