...
- **Platform facts** — endianness, word width and hardware features — are implementation-defined and belong to platform declarations consumed by CCS ([Platform Predicates](platform-predicates.md)).
- **The `ulp_min` of each representation family** is implementation-defined ([Numeric Selection §2](numeric-selection.md#2-the-selection-objective)).
