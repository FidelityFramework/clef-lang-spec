...
6. **Quire sizing-pass obligation.** The `n²/2` *width* is settled; the per-product-fit verification, allocation, and coeffect-emission *pass* (§10.2.1) is downstream and must be fully defined.
...
- Jonnalagadda, A. A., Thotli, R., & Gustafson, J. L. (2026). *Closing the Gap Between Float and Posit Hardware Efficiency.* arXiv:2603.01615 *(unverified: a future-dated identifier; cited for the qualitative b-posit direction only, not for load-bearing figures — see §7)*. (Bounded-regime b-posit; shared decode/MUX across widths.)
