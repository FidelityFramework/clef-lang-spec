...
Not a Clef construct. F# quotes a method argument implicitly where the parameter carries `[<ReflectedDefinition>]` and has the core library's `Expr<_>` type; Clef has no `ReflectedDefinition`, no run-time quotation value and no implicit quotation at a call ([Special Attributes and Types](special-attributes-and-types.md)). A quotation is written `<@ expr @>` where it is meant, and is compile-time data the compiler reads ([Expressions, Quoted Expressions](expressions.md)).
